...
const PensionInvestmentDetails = require("../../../Models/Retirements/Partner/Partner_PensionAccount_Beneficiaries");
const PensionInvestmentSchema = require("../../../Schema/Retirements/Partner/Partner_PensionAccount_Beneficiaries");
...
MyRouter.post("/Add-Partner-PensionAccount-Beneficiaries", async (req, res) => {
  const NewClientPensionInvestment = req.body;
  const { error } = PensionInvestmentSchema(NewClientPensionInvestment);
  if (error) {
    res.status(404).send({ message: error.details[0].message });
  } else {
    let AddPensionInvestment = new PensionInvestmentDetails(
...
MyRouter.patch(
  "/Update-Partner-PensionAccount-Beneficiaries/:Email/:_id",
  async (req, res) => {
    const UpdatePensionInvestment = await PensionInvestmentDetails.findOne({
      Email: req.params.Email,
      _id: req.params._id,
    });

    (UpdatePensionInvestment.Email = req.body.Email),
      (UpdatePensionInvestment.NominatedBeneficiary =
        req.body.NominatedBeneficiary),
      (UpdatePensionInvestment.NominationType = req.body.NominationType),
      (UpdatePensionInvestment.No_ofBeneficiaries =
        req.body.No_ofBeneficiaries),
...
MyRouter.delete(
  "/Delete-Partner-PensionAccount-Beneficiaries/:Email/:_id",
  async (req, res) => {
    const DeletePensionInvestment = await PensionInvestmentDetails.findOne({
      Email: req.params.Email,
      _id: req.params._id,
    });
    try {
