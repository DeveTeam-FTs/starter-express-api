...
MyRouter.patch("/Update-Client-SMSFForm/:Email", async (req, res) => {
  const UpdateClient = await Client_SMSFFormDetails.findOne({
  ...
  (UpdateClient.Email = req.body.Email),
    ...
    (UpdateClient.SMSFTrusteeName = req.body.SMSFTrusteeName)
...
MyRouter.delete("/Delete-Client-SMSFForm/:Email", async (req, res) => {
  const DeleteClient = await Client_SMSFFormDetails.findOne({
    Email: req.params.Email,
  });
  try {
