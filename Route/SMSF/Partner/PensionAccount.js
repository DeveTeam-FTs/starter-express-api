...
MyRouter.patch("/Update-Client-PensionAccounts/:Email", async(req, res) => {
    const UpdateClient = await Client_PensionAccountsDetails.findOne({ Email: req.params.Email});
...
MyRouter.delete("/Delete-Client-PensionAccounts/:Email", async(req, res) => {
    const DeleteClient = await Client_PensionAccountsDetails.findOne({ Email: req.params.Email});
    try{
